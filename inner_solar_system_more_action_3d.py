...
class SolarSystemAnimation3D:
    ...
    def init_positions(self):
        ...
        self.belt_inclination = np.random.uniform(-15, 15, self.num_asteroids)  # Increased inclination range
        ...
        # Initialize Hildas with 3D movement
        self.hilda_angles = []
        ...
        # Initialize Trojans with 3D libration
    ...
    def update(self, frame):
        ...
        for planet, pos in self.planet_positions.items():
            ...
            self.ax.scatter(pos['x'], pos['y'], pos['z'], c=color, s=size)
        ...
        self.ax.set_title(f'3D Solar System Animation - Frame {frame}', pad=20)
...
for version, style in styles.items():
    ...
    filename = os.path.join(OS_PATH, f"output/inner_solar_system_more_action_3d_{version}.mov")
    solar_system.save4k(filename)
    filename = os.path.join(OS_PATH, f"output/inner_solar_system_more_action_3d_{version}.gif")
